...
#' lapply(X = docs, FUN = function(X){ str(getTracking.data(X))})
...
getMom.operatorshiftdefs <- function(doc){
  # doc <- xml2::read_xml(momfiles[2])
  xpt1 <- ".//d1:OperatorShiftDefinition"
  rnode1 <-  xml2::xml_find_first(doc, xpt1)

  if(!is.na(rnode1)){
    # Create XPath expressions for each child node ( deliverydefinition data entry)
    chnames <- xml2::xml_name(xml2::xml_children(rnode1))
    chnsub <- xml2::xml_length(xml2::xml_children(rnode1))

    # Those not having having childs
    chnames1 <- chnames[chnsub == 0]
    xpath_expressions1 <- mapply(x = chnames1,
                                 FUN = function(x){ paste0(".//d1:OperatorShiftDefinition/d1:", x)
                                 }
    )     %>%  unname()
    returns1 <- lapply(xpath_expressions1, function(xpath) xml2::xml_text(xml2::xml_find_all(doc, xpath)) )
