...
hprdata <- function(hprfile){
  ...
  if(length(StemKey)){
    ...
    # Object definition (Harvest site)
      objects <- sf2010r::getObjects( doc)
                                        # returns data_frame(ObjectKey, SubObjectKey, ObjectUserID, ObjectName, SubObjectName,  LoggingFormCode,  LoggingFormDesc )
...
lfiles <- list.files(path =  system.file(package = "sf2010r"),
 pattern = ".hpr", recursive = TRUE, full.names= TRUE)



hpr_iterator <- function(hprfile,y = 1, ...){
  hprfile <- list.files(hprfile, pattern = 'hpr',...)
...
tmp <- hpr_iterator(lfiles, 1:2, recursive = TRUE, full.names= TRUE)
